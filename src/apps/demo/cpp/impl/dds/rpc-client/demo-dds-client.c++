...
namespace picarro::demo::dds
{
    ClientImpl::ClientImpl(const std::string &identity, int domain_id)
        : API(identity, TYPE_NAME_FULL(This)),
          DDS_Client_Wrapper(TYPE_NAME_FULL(This), Picarro::Demo::SERVICE_ID, domain_id)
    {
    }
    ...
    {
        auto idl_greeting = idl::encoded<Picarro::Demo::Greeting>(greeting);
        this->client().say_hello(idl_greeting);
    }

    TimeData ClientImpl::get_current_time()
    {
        Picarro::Demo::TimeData idl_time_data = this->client().get_current_time();
