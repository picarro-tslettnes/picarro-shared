...
class SettingsStore (JsonReader):
    ...
    def __init__(self,
        ...
        * a per-unit configuration folder (`/etc/picarro` on UNIX or `C:/Picarro` on Windows)
        * an application-provided default settings folder (`share/settings`), presumed
        ...
        somewhere inside `/opt/picarro/share/python/`, and default configurations are
        stored in JSON files within the folder `/opt/picarro/share/settings/`. 
