...
from grpc_client    import BaseClient, ArgParser, ProtoBuf, Picarro
...
MappingChange = enums(Picarro.Signal.MappingChange)
...
class SignalClient (BaseClient):
    ...
    ```
        client = SignalClient(host, Picarro.mypackage.MySignal, ...)
    ...
    def start_watching (self, watch_all: bool = True):
        ...
        method with a `Picarro.Signal.Filter` input based on which signal slots were
...
class MappedSignalClient (SignalClient):
    ...
    def _on_signal (self, msg):
        signalname, data = self._signal_item(msg)
        try:
            change = Picarro.Signal.MappingChange[msg.change]
        except IndexError:
            change = msg.change
