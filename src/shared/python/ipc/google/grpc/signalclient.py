...
from .client                  import Client, ArgParser, ProtoBuf, Picarro
...
class SignalClient (Client, SignalReceiver):
    ...
    ```
        def my_signal_handler(change: Picarro.Signal.MappingChange,
                              key   : str,
                              data  : DataType1):
    ...
    ```
        client = SignalClient(host, signal_type = Picarro.mypackage.MySignal, ...)
