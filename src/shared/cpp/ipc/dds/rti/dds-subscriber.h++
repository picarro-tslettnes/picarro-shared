...
namespace picarro::dds
{
    ...
    protected:
        ...
        template <class T>
        using Handler = std::function<void(picarro::signal::MappingChange, const T &)>;
    ...
    public:
        ...
        template <class T>
        inline DataReaderRef<T>
        create_signal_reader(picarro::signal::Signal<T> *signal,
                             ...
                             bool sync_latest = false)
        {
            return this->create_reader<T>(
                topic_name,
                std::bind(&picarro::signal::Signal<T>::emit, signal, std::placeholders::_2),
                reliable,
                sync_latest);
        }
    ...
    private:
                     ...
                     const Handler<T> &handler)
        {
            ...
            for (const auto &sample : samples)
            {
                ...
                {
                    handler(picarro::signal::MAP_UPDATE, sample.data());
                }
                         ...
                         ::dds::sub::status::InstanceState::not_alive_disposed())
                {
                    ...
                    handler(picarro::signal::MAP_REMOVAL, std::move(key_holder));
                }
                else
                {
